...
def login_usuario(correo, password):
    try:
        return db.select('login', where = 'correo=$correo and password=$password' , vars=locals())[0] #selecciona el primer registro que coincida con el id dado en el parametro
...
def login_estacionamiento(correo, password):
    try:
        return db.select('parks', where = 'correo=$correo and password=$password' , vars=locals())[0] #selecciona el primer registro que coincida con el id dado en el parametro
    except Exception as e:
